...
impl Shell {
    ...
    pub fn get_stdin(&mut self, job: usize) -> &mut ChildStdin {
        ...
        (&mut (&mut self.jobs.get_mut(&job).unwrap().process).stdin)
            .as_mut()
            .unwrap()
    }
    ...
    pub fn get_stdout(&mut self, job: usize) -> &mut ChildStdout {
        // Could panic at any moment...
        (&mut (&mut self.jobs.get_mut(&job).unwrap().process).stdout)
            .as_mut()
            .unwrap()
    }
    ...
    pub fn get_stderr(&mut self, job: usize) -> &mut ChildStderr {
        // Could panic at any moment...
        (&mut (&mut self.jobs.get_mut(&job).unwrap().process).stderr)
            .as_mut()
            .unwrap()
    }
}
